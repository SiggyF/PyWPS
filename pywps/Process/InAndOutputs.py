...
class LiteralInput(Input):
    ...
    def __init__(self,identifier,title,abstract=None,
                ...
                uoms=(),allowedValues=("*"),spacing=None,default=None):
        """Class constructor"""
        ...
        if type(allowedValues) == types.StringType:
            self.values = (allowedValues)
        elif type(allowedValues) == types.ListType:
            self.values = allowedValues
...
class ComplexInput(Input):
    ...
    def storeData(self,data):
        ...
        logging.info('storing {}, with mime type {}'.format(fout.name, self.format['mimetype']))
        istext = False
        if self.format["mimetype"].lower().split("/")[0] == "text":
            istext = True
        if self.format["mimetype"].lower() in {"application/xml", "application/json"}:
            istext = True
        if not istext:
            logging.info("converting {} to binary".format(fout.name))
...
class ComplexOutput(Output):
    ...
    def setValue(self, value):
        ...
        elif value.__class__.__name__ in ('StringIO', 'StringO', 'BytesIO'):
            import tempfile
            from os import curdir
